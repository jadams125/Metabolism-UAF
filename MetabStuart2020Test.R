...
Final.Strt.2020 <- Final.Strt.2020 %>%
...
Final.Strt.2020 <- Final.Strt.2020 %>%
  dplyr::rename(light = Calibrated.Par.Value)

Final.Strt.2020 <- Final.Strt.2020 %>%
  dplyr::rename(discharge = Strt1comb.2020.pred.strt1.Q)



#Pressure?
moos.2020.atmo.url <- "https://drive.google.com/drive/u/2/folders/10iEEQn5LhX3sxD2rcwozzAjuxmSuWKuh"
moosatmo <- drive_get(as_id(moos.2020.atmo.url))
test1moos2020_glist <- drive_ls(moosatmo, pattern = "20574421_MOOS_atmo.csv")
walk(test1moos2020_glist$id, ~ drive_download(as_id(.x), overwrite = TRUE))
moos2020atmo.Data <- read.csv("20574421_MOOS_atmo.csv", 
                              skip = 1, header = TRUE)
...
names(moos2020atmo.Data)[3]= c("pressure.air")
names(moos2020atmo.Data)[2]= c("dateTimePre")

moos2020atmo.Data$pressure.air <- moos2020atmo.Data$pressure.air * 10

moos2020atmo.Data$DateTime <- as.POSIXct(moos2020atmo.Data$dateTimePre, format="%m/%d/%y %I:%M:%S %p", tz="America/Anchorage")



moos2020atmo.Data$DateTime <- as.POSIXct(moos2020atmo.Data$DateTime)
Final.Strt.2020$DateTime <- as.POSIXct(Final.Strt.2020$DateTime)


Final.Strt.2020 <- plyr::join(moos2020atmo.Data, Final.Strt.2020, by = "DateTime")

Final.Strt.2020 <- na.omit(Final.Strt.2020)


Final.Strt.2020$ModelDOSat <- calc_DO_sat(Final.Strt.2020$temp.water, Final.Strt.2020$pressure.air, model = "garcia-benson")



Final.Strt.2020$DO.sat <- Final.Strt.2020$ModelDOSat
...
test.strt$DateTime[test.strt$DateTime == "2020-06-24 12:45:00"] <- NA


test.strt$DateTime[test.strt$DateTime == "2020-06-24 11:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 12:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 12:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 12:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 12:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 12:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 13:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 13:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 13:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 13:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 14:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 14:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 14:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-06-24 14:45:00"] <- NA

test.strt$DateTime[test.strt$DateTime == "2020-07-07 10:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-07 10:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-07 10:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-07 11:00:00"] <- NA


test.strt$DateTime[test.strt$DateTime == "2020-07-22 11:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 11:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 12:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 12:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 12:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 12:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 13:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 13:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 13:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 13:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-22 14:00:00"] <- NA


test.strt$DateTime[test.strt$DateTime == "2020-07-29 17:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-29 17:15:00"] <- NA


test.strt$DateTime[test.strt$DateTime == "2020-07-29 17:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-07-29 17:15:00"] <- NA

test.strt$DateTime[test.strt$DateTime == "2020-08-03 12:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-08-03 12:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-08-03 12:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-08-03 12:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-08-03 13:00:00"] <- NA

test.strt$DateTime[test.strt$DateTime == "2020-08-19 12:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-08-19 13:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-08-19 13:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-08-19 13:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-08-19 13:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-08-19 14:00:00"] <- NA


test.strt$DateTime[test.strt$DateTime == "2020-09-01 12:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-01 12:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-01 12:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-01 12:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-01 13:00:00"] <- NA

test.strt$DateTime[test.strt$DateTime == "2020-09-24 10:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-24 10:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-24 11:00:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-24 11:15:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-24 11:30:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-24 11:45:00"] <- NA
test.strt$DateTime[test.strt$DateTime == "2020-09-24 12:00:00"] <- NA



test.strt <- na.omit(test.strt)
...
Predict <- predict_metab(mm.test)
plot1 <- plot_metab_preds(mm.test)
plot1

predict_DO(mm.test) %>% head()


plot_DO_preds(mm.test)
mcmc <- get_mcmc(mm.test)
...
start <- as.Date(data.strt.mm$solar.time[1000])
end <- as.Date(data.strt.mm$solar.time[1500])




plot_DO_preds(mm.test, date_start = start, date_end = end)
...
plot(ODO.Mgl.vaul.2020, vaul.exo$DateTime, main = "Vault, Unburned")
...
plot(ODO.Mgl.strt.2020, strt.exo$DateTime, main = "Stuart Creek, 2020 Optical Dissilved Oxygen", ylab = "ODO (mg/L)")
 
install.packages("ggiraph")
library(ggiraph)

strtODO <- ggplot(dat = strt.exo)+
  geom_point_interactive(aes(x=DateTime, y=ODO.mgL), size = 3)+
  ggtitle("Stuart 2020 Opitical Dissolved Oxygen")+
  labs(x = "Date", y = "ODO (mg/L)")
strtODO


girafe(ggobj = strtODO)
...
vaulODO <- ggplot(vaul.exo, aes(x=DateTime, y=ODO.mgL))+
  geom_point(size = 3)+
  ggtitle("Vault 2020 Opitical Dissolved Oxygen")+
  labs(x = "Date", y = "ODO (Mg/L)")
...
test1 <- grid.arrange(strtODO, vaulODO, ncol=2)


Q1 <- quantile(vaul.exo$Apperance, .25)
Q3 <- quantile(vaul.exo$Apperance, .75)
IQR <- IQR(vaul.exo$Apperance)

no_outliers <- subset(vaul.exo, vaul.exo$Apperance > (Q1 - 1.5*IQR) & vaul.exo$Apperance < (Q3 + 1.5*IQR))
dim(no_outliers)
...
onedaystrt$DateTime <- as.POSIXct(onedaystrt$DateTime)

onedaystrt <- onedaystrt %>%  filter(DateTime <= "2020-08-07 23:45:00") %>%  filter(DateTime >= "2020-08-01 0:00:00")

onedaystrt <- onedaystrt[- c(2663:2684), ]



str(onedaystrt)

onedaystrt$ODO.mgL <- as.numeric(onedaystrt$ODO.mgL)

ODO.Mgl.strt.2020 <- aggregate(ODO.mgL ~ DateTime, onedaystrt, mean )

ODO.psat.strt.2020 <- aggregate(as.numeric(ODO.Psat) ~ DateTime, onedaystrt, mean )

#Where is DO saturated data 

Temp.C.2020.strt <- aggregate(Temp.C ~ DateTime, onedaystrt, mean )
